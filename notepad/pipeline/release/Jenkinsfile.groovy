podTemplate(label: 'release',
        ...
        ])
        {
          ...
          git clone git@github.com:adidhab/notepad.git
          cd notepad
          git config --global user.email "jenkins@abhishekdhabriya.com"
          git config --global user.name "Jenkins"
          mvn -B -s /etc/maven/settings.xml release:prepare "-DreleaseVersion=${
                            RELEASE_VERSION
                        }" "-DdevelopmentVersion=${NEW_VERSION}" release:perform -Darguments="-DskipTests"
        """
                    }
                }
